...
export function processEmotes(input, isMod, mapping) {
	...
	message.forEach((word, index) => {
		const validSiteURL = /^http[s]?:\/\/(iab\.25edinburgh\.org|localhost:8080|github\.com\/25Edinburgh\/its-always-bob)\/([a-zA-Z0-9#?=&\/\._]*)$/i;
		if (mapping[word]) {
			formatedMsg.push(
			...
			const isGithub = data[1] == 'github.com/25Edinburgh/its-always-bob';
			const gameURL = data[2].startsWith('game/');

			formatedMsg.push(
				<a
					key={index}
					href={isGithub ? 'https://github.com/25Edinburgh/its-always-bob/' + data[2] : gameURL ? '/game/' + data[2].substring(5) : '/' + data[2]}
					className="shio-link"
